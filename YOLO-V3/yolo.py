...
COLORS = np.random.randint(0, 255, size=(len(LABELS), 3),
	dtype="uint8")
def displayVehicleCount(image, total_vehicles, each_vehicle,time_delay):
	cv2.putText(
		image, #Image
		'Detected Vehicles: ' + str(total_vehicles), #Label
		(20, 20), #Position
	...
	if(total_vehicles == 0):
		cv2.putText(
			image, #Image
			'The Light is Green for other lanes', #Label
			(20, 100), #Position
			cv2.FONT_HERSHEY_SIMPLEX, #Font
			0.5, #Size
			(0, 0xFF, 0), #Color
		...
		cv2.putText(
			image, #Image
			'The Light is Red for this lane', #Label
			(20, 120), #Position
			cv2.FONT_HERSHEY_SIMPLEX, #Font
			0.5, #Size
			(0, 0xFF, 0), #Color
		...
		cv2.putText(
			image, #Image
			"Delay: " + str(time_delay)+ " seconds", #Label
			(20, 110), #Position
			cv2.FONT_HERSHEY_SIMPLEX, #Font
			0.8, #Size
			(0, 0xFF, 0), #Color
...
if len(idxs) > 0:
	...
	for i in idxs.flatten():
		...
		cv2.putText(image, text, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX,
			0.5, color, 2)
time_delay = 0
list1 = each_vehicle.keys()
for z in list1:
	if(z == 'car'):
		time_delay = time_delay + each_vehicle[z]*5
	elif(z == 'truck'):
		time_delay = time_delay + each_vehicle[z]*10

print("Time delay in image is: " + str(time_delay) + " seconds")
displayVehicleCount(image, total_vehicles, each_vehicle, time_delay)
count = random.randint(1,1000)
# show the output image
cv2.imwrite('output_images/Image%d.jpg'%count, image)
